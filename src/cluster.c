...
int clusterProcessPacket(clusterLink *link) {
    ...
    // 根据不同消息类型, 解析成不一样的结构体.
    // 不管任何消息, redis 后面都会调用 clusterSendPing 函数以PONG形式返回.
    ...
    if (type == CLUSTERMSG_TYPE_PING || type == CLUSTERMSG_TYPE_MEET) {
         ...
         * of the message type. */
        // 对于未知节点的MEET消息, 也必须处理.
        // clusterProcessGossipSection 函数会依次处理 Ping-Pong 消息中包含的多个消息体, 这样收到PING/PONG消息的节点
        // 可以根据消息体的数据, 更新本地记录的对应节点的信息.
        if (!sender && type == CLUSTERMSG_TYPE_MEET)
...
void clusterLinkConnectHandler(connection *conn) {
    ...
    // 当一个节点收到PING消息时, 就会调用 clusterReadHandler 函数处理.
    connSetReadHandler(conn, clusterReadHandler);
...
void clusterReadHandler(connection *conn) {
    ...
    while (1) {  // 持续读取收到的数据
        ...
        // 读取到完整的消息, 调用 clusterProcessPacket 处理这条消息
...
void clusterCron(void) {
    ...
    while ((de = dictNext(di)) != NULL) {
        ...
        if (node->link == NULL) {
            ...
            // 重新建立连接, 并设置回调函数：clusterLinkConnectHandler
...
/**
 * 处理 CLUSTER 命令, 它根据 CLUSTER 命令携带的不同选项,
 * 执行不同的代码分支.
 */
void clusterCommand(client *c) {
    ...
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr, "help")) {
        ...
        /*
         * 命令：CLUSTER  SETSLOT  <slot>  IMPORTING  <node> //<slot>表示要迁入的哈希槽, <node>表示当前负责<slot>的节点.
         * 例子：如果 slot1 在节点A上, 现要把 slot1 从A迁移到节点B, A就成为待迁出数据的源节点, B成为待迁入数据的目的节点, 此时需要
         *      在节点B上执行命令: CLUSTER  SETSLOT  slot1  IMPORTING  nodeA
         *      在节点A上执行命令: CLUSTER  SETSLOT  slot1  MIGRATING  nodeB
         * 下面分支执行的是 MIGRATING 命令, 是在节点A上运行的,
         */
        if (!strcasecmp(c->argv[3]->ptr, "migrating") && c->argc == 5) {
            // 检查节点A是否是这个slot的owner
            if (server.cluster->slots[slot] != myself) {
                addReplyErrorFormat(c, "I'm not the owner of hash slot %u", slot);
                return;
            }
            // 通过节点名称, 是否能找到"nodeB"节点.
            if ((n = clusterLookupNode(c->argv[4]->ptr)) == NULL) {
                addReplyErrorFormat(c, "I don't know about node %s",
                                    (char *) c->argv[4]->ptr);
                return;
            }
            // migrating_slots_to 数组中迁出 slot, 设置成NodeB
             ...
             *      在节点B上执行命令: CLUSTER  SETSLOT  slot1  IMPORTING  nodeA
             *      在节点A上执行命令: CLUSTER  SETSLOT  slot1  MIGRATING  nodeB
             * 下面分支执行的是 IMPORTING 命令, 是在节点B上运行的
             */
            ...
            // 通过节点名称, 是否能找到"nodeA"节点.
            if ((n = clusterLookupNode(c->argv[4]->ptr)) == NULL) {
                addReplyErrorFormat(c, "I don't know about node %s",
                                    (char *) c->argv[4]->ptr);
                return;
            }
            // importing_slots_from 数组中迁入 slot, 设置成NodeA
        ...
        /*
         * 命令：CLUSTER  GETKEYSINSLOT  <slot>  <count>, 获取待迁出的keys, 第一个参数表示要迁移的slot, 第二个参数表示要迁移的key的数量.
         * 例子：如果 slot1 在节点A上, 现要把 slot1 从A迁移到节点B, A就成为待迁出数据的源节点, B成为待迁入数据的目的节点, 此时需要
         *      在节点B上执行命令: CLUSTER  SETSLOT  slot1  IMPORTING  nodeA
         *      在节点A上执行命令: CLUSTER  SETSLOT  slot1  MIGRATING  nodeB
         * 下面分支执行的是 IMPORTING 命令, 是在节点B上运行的
        ...
        // 从 CLUSTER GETKEYSINSLOT 命令中解析获取 slot 参数赋值给slot
        if (getLongLongFromObjectOrReply(c, c->argv[2], &slot, NULL) != C_OK)
            return;
        ...
        /*
         * Avoid allocating more than needed in case of large COUNT argument and smaller actual number of keys.
         */

        // 获取迁移slot中实际的key数量
        unsigned int keys_in_slot = countKeysInSlot(slot);
        // 如果实际的key数量小于maxkeys, 将maxkeys更新为时间的key数量
        if (maxkeys > keys_in_slot) maxkeys = keys_in_slot;
        // 给key分配空间
        keys = zmalloc(sizeof(robj *) * maxkeys);
        // 获取实际的key
        numkeys = getKeysInSlot(slot, keys, maxkeys);
        // 将key返回给客户端, 后续客户端就可以获得一定数量的要迁移的key, 以便执行实际的迁移操作.
...
/**
 * 集群的数据迁入节点, 收到数据迁出节点发送的 RESTORE-ASKING 命令, 就会调用此函数.
 * 主要分为三步:
 * 1) 解析命令参数, 包括标记REPLACE、key的过期时间ttl、key的LRU标记、key的LFU标记..., 对参数做检查
 * 2) 检查迁移 key 的 value 的序列化结果, 调用 verifyDumpPayload 函数检测 RDB 版本和 CRC 校验和, 没问题解析出实际 value 类型和 value 实际值.
 * 3) 调用 dbAdd 函数, 把解析得到 key 和 value 写入目的节点的数据库(如果是REPLACE标记, 会先调用dbDelete函数删除当前库已经存在的key)
 *
 * @param c
 */
 ...
 *
 * 执行 MIGRATE 命令, 可选项包含了目的节点的IP、端口号、数据库编号、要迁移的Key、迁移超时的时间:
 * MIGRATE host port "" dbid timeout [COPY | REPLACE | AUTH password | AUTH2 username password] KEYS key1 key2 ... keyN.
 *
 * COPY选项, 如果目的节点已经存在待迁移的Key则报错, 如果目的节点不存在待迁移的Key, 正常迁移后, 删除源节点的Key;
 * REPLACE选项, 无论目的节点是否存在待迁移的Key, 都会正常执行迁移, 并覆盖已经存在的Key;
 */
void migrateCommand(client *c) {
    ...
    // 分配两个数组ov和kv, 初始大小等于MIGRATE命令中要迁移的Key数量.
    ov = zrealloc(ov, sizeof(robj *) * num_keys); //分配ov数组, 它保存要迁移的value.
    kv = zrealloc(kv, sizeof(robj *) * num_keys); //分配kv数组, 它保存要迁移的Key.
    int oi = 0;
    ...
    // 对要迁移的key, 往buffer中填充命令、key和value.
    for (j = 0; j < num_keys; j++) {
        long long ttl = 0;
        ...
        // 集群模式下, 填充RESTORE-ASKING命令, 用来发送给目的节点
        ...
        // 调用createDumpPayload函数序列化value, 并加入RDB版本和CRC校验和
    ...
    // 对发送的每个Key, 读取目的节点的返回结果. 如果返回结果中有报错信息,
    // 那么就会进行相应的处理
    for (j = 0; j < num_keys; j++) {
...
/**
 * 计算能执行这个命令的集群节点, 其主要分为三个步骤:
 * 1) 使用multiState结构体封装收到的命令;
 * 2) 针对收到的每个命令, 逐一检查这些命令访问的 key 所在的 slots;
 * 3) 根据 slot 的检查结果返回 hashslot、error_code 和相应的集群节点;
 *
 * @param c 发送命令的客户端
 * @param cmd 当前命令
 * @param argv 命令的参数
 * @param argc 命令的参数
 * @param hashslot 命令访问的Key所在的slot(哈希槽)
 * @param error_code 执行后的错误代码
 * @return 能够处理该命令的集群节点
 */
clusterNode *
getNodeByQuery(client *c, struct redisCommand *cmd, robj **argv, int argc, int *hashslot, int *error_code) {
    ...
    /*
     * 因为 redis server 支持 MULTI 命令, 它表示事务, 会缓存后续的命令.
     * 对于集群节点来说, 它也需要支持 MULTI 命令. 为了通用, 这里不管是处理真正的
     * MULTI 命令后续的命令, 还是常规的单条命令, 都使用了 multiState 结构体, 封装当前要查询的命令
     */
    if (cmd->proc == execCommand) {
        /* If CLIENT_MULTI flag is not set EXEC is just going to return an error. */
        if (!(c->flags & CLIENT_MULTI)) return myself;
        // MULTI 命令后缓存的其它命令并不会立即执行, 而是会等到 EXEC 命令执行时才会执行,
        // 对于这种情况, 当前集群节点才会在收到EXEC命令时, 从从客户端变量c中获取mstate.(在processCommand函数中就已经创建好了)
        ms = &c->mstate;
    } else {
        /* In order to have a single codepath create a fake Multi State
         * structure if the client is not in MULTI/EXEC state, this way
         * we have a single codepath below. */
        ms = &_ms;           // 如果是其他命令, 也使用multiState结构体封装命令
        _ms.commands = &mc;
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        // 调用 getKeysFromCommand 函数获取命令中的Key位置和Key个数
        ...
        for (j = 0; j < numkeys; j++) {
            ...
            if (firstkey == NULL) {
                /* This is the first key we see. Check what is the slot and node. */
                firstkey = thiskey;
                slot = thisslot;
                // 查找Key所属的slot对应的集群节点
                 ...
                 * error to the client. */
                // 匹配到的集群节点为空, 将error_code置为CLUSTER_REDIR_DOWN_UNBOUND, 返回给客户端.
                if (n == NULL) {
                ...
                // 匹配到的集群节点就是本节点, 且key所属的slot正在迁出, 设置migrating_slot为1
                if (n == myself && server.cluster->migrating_slots_to[slot] != NULL) {
                    migrating_slot = 1;
                }
                    // key所属的slot正在迁入, 则设置importing_slot为1
                else if (server.cluster->importing_slots_from[slot] != NULL) {
                    importing_slot = 1;
                }
            } else {
                // 表示说当前命令要查询Keys不在同一个Slot, 会报错, 将错误码置为: CLUSTER_REDIR_CROSS_SLOT 返回给客户端.
            ...
            // 代码走到这里, 说明已经查找到当前命令访问的Key所在的slot, 和它对应的集群节点.
            // 但是若key所属slot正在迁出或迁入, 并且当前访问的key不在本地数据库, 那么增加missing_keys的大小, 记录缺失的Key数量.
    ...
    /**
     * 根据 slot 的检查结果返回 hashslot、error_code 和相应的集群节点, 情况很多, 主要有4个情况
     */
    ...
    /*
     * 2.命令访问 key 所属的 slot 正在做数据迁出或迁入, 而且当前命令就是用来执行数据迁移的 MIGRATE 命令,
     *   那么 getNodeByQuery 函数会返回当前节点.
     */
    if ((migrating_slot || importing_slot) && cmd->proc == migrateCommand)
        return myself;

    /*
     * 3.命令访问 key 所属的 slot 正在做数据迁出, 并且命令访问的 key 在当前节点数据库中缺失了, 即上面的 missing_keys 大于 0.
...
// 根据参数 error_code 的不同值, 执行相应的代码分支, 主要是把 key 所属 slot 对应集群节点的情况返回给客户端,
// 从而让客户端根据返回的信息作出相应处理.
void clusterRedirectClient(client *c, clusterNode *n, int hashslot, int error_code) {
    if (error_code == CLUSTER_REDIR_CROSS_SLOT) {
        ...
        // 返回 MOVED/ASK 命令, 并把 key 所属的 slot、slot 实际所属的节点 IP 和端口号，返回给客户端.
