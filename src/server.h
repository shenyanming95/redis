 ...
 * 划分成多个位域, 每个位域占用一定的bits, 因此一个数据类型就可以定义多个变量, 从而节省内存开销.
 *
 * type、encoding 和 lru 三个变量它们的数据类型都是 unsigned. 已知 unsigned 类型是 4 字节, 那么
 * 这三个变量分别占用了一个 4 字节中的 4bits、4bits 和 24bits. 相较于使用三个变量（每个变量用一个 4 字节的 unsigned 类型定义）
 * 使用位域定义方法可以让三个变量只用 4 字节, 最后就能节省 8 字节的开销.
 */
typedef struct redisObject {
    // redisObject 的数据类型, 是用户在 Redis 中保存的数据类型, 包括 String、List、Hash、Set、sorted Set. 4个bits.
    unsigned type: 4;
...
typedef struct zskiplistNode {
    ...
    // 节点的层数数组, 保存每层上的前驱指针和跨度
    struct zskiplistLevel {
        struct zskiplistNode *forward;
        // 跨度: 记录结点在某一层上的*forward指针和该指针指向的结点之间, 跨越了 level0 上的几个结点.
        unsigned long span;
    } level[];
} zskiplistNode;
...
struct redisServer {
    ...
    int supervised;                 /* 是否使用 upstart 或是 systemd 这两种守护进程的管理程序来管理 Redis */
    int supervised_mode;            /* See SUPERVISED_* */
    int daemonize;                  /* 是否要设置 Redis 以守护进程方式运行, true表示是 */
